...
class Performance(object):
    ...
    def calculate_performance(self):
                ...
                self.perfcalc.append(
                    ''.join(
                        ('\tLargest population is over 95% of total ',
                         'for selected events.\n',
                         '\tCalculation continued using largest population.\n',
                         )))
            else:
                self.perfcalc.append(
                    ''.join(
        ...
        if not stable:
            self.perfcalc.append(
                ''.join(
                    ('\nNo opponent cycles in selected events like: A plays ',
                     'B, B plays C, C plays A.\n\n',
                     'This is a 3-cycle and when present, the usual case, ',
                     'ensures the iteration will converge.',
                     '\n\nAn n-cycle, n>3, exists but this does not ensure ',
                     'the iteration will converge: it depends on the pattern ',
                     'of results of the games in the cycle.  This case seems ',
                     'to be one which does not converge.\n\nThe workaround ',
                     'is attach a 3-cycle, using two artifical player names, ',
                     'to an existing player who plays games against only one ',
                     'opponent if possible.  The three added games should be ',
                     'draws.',
                     )))
            return
        self.calculation = s_calculation
        self.perfcalc.append(''.join(
            ('Iterations used: ',
             str(iterations),
             '      Delta: ',
             str(delta),
        ...
        max_performance = round(
            max([p.get_calculated_performance()
        ...
        player_order = sorted(
            [(self.names[p][0],
              ...
              p,
              self.names[p][-1],
              -(round(pr.get_calculated_performance()) - max_performance),
