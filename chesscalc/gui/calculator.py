...
class Calculator(ExceptionHandler):
    ...
    def calculate_performance(self):
        ...
        for cfpc, fpg, fpc in self.fixed_performance[1:]:
            ...
            report_lines.append(''.join(
                ('\n\n\nThe following performance calculation includes all ',
                 'players but the performances calculated earlier by ignoring ',
                 'all players with less than ',
                 str(cfpc + 1),
                 ' opponents are used as fixed values for this calculation.',
                 '\n',
                 )))
            for s, v in rlines:
                report_lines.append(v)
        ...
        report_lines = []
        report_lines.append(''.join(
            ('\n\n\n\nThe performances calculated for each player are listed ',
             'in ascending order of number of opponents for ignoring a player ',
             'to derive a population.  The first line for each player gives ',
             'the calculated performance with all players included and the ',
             'second gives the performance if the player was not excluded for ',
    ...
    def find_populations(self):
        ...
        if len(self.performance.subpopulations[-1][-1]) > 1:
            ...
            lines.append(''.join(
                ('The population does not fracture into sub-populations when ',
                 'players are ignored in order by ascending number of ',
                 'opponents played.  Maximum opponents is ',
                 str(len(self.performance.subpopulations[-1]) - 1),
                 '.\n')))
        self.report.insert(tkinter.END, '\n'.join(lines))
